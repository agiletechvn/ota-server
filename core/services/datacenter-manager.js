...
// one mini app can contain many Module bundle files
proto.getPackageBundle = function(packageHash) {
  // later we can use other information
  var packageInfo = this.getPackageInfo(packageHash);
  var moduleFiles = fs
    .readdirSync(packageInfo.contentPath)
    .filter(file => file.endsWith('.jsbundle'));

  var base = '';
  moduleFiles.forEach(
    moduleFile =>
      (base += fs.readFileSync(path.join(packageInfo.contentPath, moduleFile)))
  );
  return base;
};
    ...
    return self.validateStore(packageHash).then(isValidate => {
      if (!force && isValidate) {
        ...
        return common.createEmptyFolder(packageHashPath).then(() => {
          return common.move(sourceDst, contentPath).then(() => {
