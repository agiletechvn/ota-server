...
router.patch(
  ...
  (req, res, next) => {
    ...
    accountManager
      .ownerCan(uid, appName)
      .then(col => {
        return deployments.renameDeloymentByName(
          deploymentName,
          col.appid,
          name
        );
      ...
      .then(col => {
        ...
        return Promise.all([
          ...
          .spread((sourceDeploymentInfo, destDeploymentInfo) => {
            if (!sourceDeploymentInfo) {
              throw new AppError.AppError(
                `${sourceDeploymentName}  does not exist.`
              );
            }
            if (!destDeploymentInfo) {
              throw new AppError.AppError(
                `${destDeploymentName}  does not exist.`
              );
            }
            //clear cache if exists.
            if (_.get(config, 'common.updateCheckCache', false) !== false) {
          ...
          .spread((sourceDeploymentInfo, destDeploymentInfo) => {
            ...
            return packageManager.promotePackage(
...
var rollbackCb = function(req, res, next) {
  ...
  accountManager
    ...
    .then(dep => {
      ...
      return packageManager.rollbackPackage(
...
router.post('/', middleware.checkToken, (req, res, next) => {
  ...
  if (platformName == _.toLower(constName.REACT_NATIVE_NAME)) {
    ...
    return res
      .status(406)
      .send('Please input platform [React-Native|Cordova]!');
  }
  ...
  appManager
    ...
    .then(appInfo => {
      ...
      return appManager
        .addApp(uid, appName, os, platform, req.users.identical)
        .then(() => {
          return {
            name: appName,
