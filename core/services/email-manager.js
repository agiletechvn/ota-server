...
proto.sendMail = function(options) {
  return new Promise((resolve, reject) => {
    if (!_.get(options, 'to')) {
      return reject(new AppError.AppError('To is a mandatory parameter'));
    }
...
proto.sendRegisterCode = function(email, code) {
  return proto.sendMail({
    to: email,
    html: `<div>The verification code you received is: <em style="color:red;">${code}</em>  Valid within 20 minutes</div>`
  });
};
