...
proto.downloadPackageAndExtract = function(
  ...
  return dataCenterManager.validateStore(packageHash).then(isValidate => {
    if (isValidate) {
      ...
      return common
        .createFileFromRequest(downloadURL, `${workDirectoryPath}/${blobHash}`)
        .then(download => {
          return common
            .unzipFile(
              `${workDirectoryPath}/${blobHash}`,
              `${workDirectoryPath}/current`
            )
            .then(outputPath => {
  ...
  return models.PackagesDiff.findOne({
    ...
    return common
      .createFileFromRequest(
        downloadURL,
        `${workDirectoryPath}/${diffManifestBlobHash}`
      )
      .then(() => {
        var originContentPath = dataCenter.contentPath;
        var originManifestJson = JSON.parse(
          fs.readFileSync(dataCenter.manifestFilePath, 'utf8')
        );
        ...
        return self
          .zipDiffPackage(fileName, files, originContentPath, hotCodePushFile)
          .then(data => {
...
proto.createDiffPackagesByLastNums = function(packageId, num) {
  var self = this;
  return models.Packages.findById(packageId).then(originalPackage => {
...
proto.createDiffPackages = function(originalPackage, destPackages) {
  if (!_.isArray(destPackages)) {
    return Promise.reject(
      new AppError.AppError('The second argument must be an array')
    );
  }
...
proto.releasePackage = function(
  ...
  var isDisabled = packageInfo.isDisabled; //download immediately
  var rollout = packageInfo.rollout;
  var isMandatory = packageInfo.isMandatory; //force update
  ...
  return new Promise((resolve, reject) => {
    ...
    .spread((sourcePack, deploymentsVersions, appFinalVersion) => {
      ...
      if (_.isBoolean(params.isDisabled)) {
        create_params.isMandatory = params.isDisabled
          ? constConfig.IS_DISABLED_YES
          : constConfig.IS_DISABLED_NO;
      } else {
    ...
    deploymentsVersions => {
      ...
      return models.Packages.findById(deploymentsVersions.current_package_id)
        ...
        .spread((currentPackageInfo, rollbackPackageInfos) => {
          ...
          throw new AppError.AppError(
            'There is no version available for rollback'
          );
        })
